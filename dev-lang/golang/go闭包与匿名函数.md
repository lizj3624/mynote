...
最开始这个地方我有个疑问：为什么在defer中的x是1而不是101呢？其实是原因是 在defer定义时已经将x的拷贝1复制给了defer, defer执行时使用的是当时defer定义时x的拷贝，而不是当前环境中x的值。
...
## 引用
1. [go 闭包与匿名函数这一篇就够了](https://www.jianshu.com/p/faf7ef7fbcf8)

2. [Go 函数式编程篇（三）：匿名函数与闭包](https://geekr.dev/posts/go-anonymous-function-and-closure)
